...
int trace_ip6_format(char *buf, size_t len, const struct rte_ipv6_hdr *ip6, uint16_t /*hdr_len*/) {
	...
	n = snprintf(buf, len, "%s > %s ttl=%hhu proto=", src, dst, ip6->hop_limits);
	n += ip_proto_format(buf + n, len - n, ip6->proto);
...
ipv4:
	...
	case RTE_BE16(RTE_ETHER_TYPE_IPV6): {
		...
		offset += sizeof(*ip6);
		n += snprintf(buf + n, sizeof(buf) - n, " / IPv6 ");
		n += trace_ip6_format(buf + n, sizeof(buf) - n, ip6, sizeof(*ip6));
		proto = ip6->proto;
	...
	default:
		n += snprintf(buf + n, sizeof(buf) - n, " type=");
		n += eth_type_format(buf + n, sizeof(buf) - n, ether_type);
		break;
	}
...
void *gr_mbuf_trace_add(struct rte_mbuf *m, struct rte_node *node, uint8_t data_len) {
...
int gr_trace_dump(char *buf, size_t len) {
	...
	if (rte_ring_dequeue(traced_packets, &data) == 0) {
		...
		n += snprintf(buf + n, len - n, "%09lu", t->ts.tv_nsec);
		n += snprintf(buf + n, len - n, " cpu %d ---------\n", t->cpu_id);

		while (t) {
			n += snprintf(buf + n, len - n, "%s:", rte_node_id_to_name(t->node_id));
