...
int trace_ipip_format(char *buf, size_t len, const void *data, uint16_t /*data_len*/) {
	const struct trace_ipip_data *t = data;
	const struct iface *iface = iface_from_id(t->iface_id);
	return snprintf(buf, len, "iface=%s", iface->name ?: "[deleted]");
}
...
next:
		if (gr_mbuf_is_traced(mbuf) || (ipip && ipip->flags & GR_IFACE_F_PACKET_TRACE)) {
			struct trace_ipip_data *t = gr_trace_add(mbuf, node, sizeof(*t));
