...
static const struct gr_field_desc route6_event_fields[] = {
...
static ssize_t serialize_route6_event(const void *obj, void *buf, size_t buf_len) {
	...
	struct gr_ip6_route tmp = {
		.dest = priv->dest,
		.vrf_id = priv->vrf_id,
		.origin = priv->origin,
	};

	ssize_t n = gr_mpack_encode(buf, buf_len, route6_event_fields, &tmp);
	if (n < 0) {
		free(nh);
		return n;
	}
...
RTE_INIT(control_ip_init) {
	...
	gr_event_serializer(GR_EVENT_IP6_ROUTE_ADD, NULL, serialize_route6_event);
	gr_event_serializer(GR_EVENT_IP6_ROUTE_DEL, NULL, serialize_route6_event);
