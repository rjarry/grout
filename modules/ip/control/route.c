...
static struct rte_fib_conf fib_conf = {
	...
	.rib_ext_sz = 0,
	.dir24_8 = {
		// rte_fib stores the nexthop ID on 8 bytes minus one bit which
		// is used to store metadata about the routing table. The
		// address mapping in userspace is guaranteed on x86_64 and
		// aarch64 to use at most 47 bits, leaving at least 17 bits of
		// headroom filled with zeroes.
...
static inline uintptr_t nh_ptr_to_id(struct nexthop *nh) {
	...
	// to store metadata about the routing table. The address mapping in
	// userspace is guaranteed on x86_64 and aarch64 to use at most 47 bits,
	// leaving at least 17 bits of headroom filled with zeroes. For clarity,
	// we explicitly fail if the MSB is not zero.
