...
perf_fingerprint() {
	...
	echo "cpu='$model' workers=${_perf_workers:-?} khz=$fmax opts=${GROUT_OPTS:-}"
}
...
perf_run() {
	...
	done
	{
		...
		' $tmp/samples | column -t \
			-C name=NODE \
			-C name=BATCHES,right \
			-C name=PACKETS,right \
			-C name=PKTS/BATCH,right \
			-C name=CYCLES/BATCH,right \
			-C name=CYCLES/PKT,right
	} > "$out"
}
