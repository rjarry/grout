...
// API version magic: 0x1981 prefix + version number in low 16 bits.
// Bump only for fundamental wire format changes, not for field additions.
#define GR_API_MAGIC UINT32_C(0x19810000)
#define GR_API_VERSION (GR_API_MAGIC | 1)

// API request header.
struct gr_api_request {
	uint32_t api_version; // Must be GR_API_VERSION
...
struct gr_api_response {
	uint32_t api_version; // Server's GR_API_VERSION
	uint32_t for_id; // matches gr_api_request.id (for out-of-order handling)
	uint32_t type; // echoes gr_api_request.type
